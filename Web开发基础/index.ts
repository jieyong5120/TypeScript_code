...
let nums: number[] = [100, 200, 300, 400, 500, 600, 700]
...
var htmlImageElement = document.querySelector('#img') as HTMLImageElement;  //类型断言
...
var htmlDivElement = document.querySelector('.a') as HTMLDivElement;
htmlDivElement.classList.add('b', 'c')
console.log('打印类选择器集合:', htmlDivElement.classList)

htmlDivElement.classList.remove('b', 'c')

let containsToClass = htmlDivElement.classList.contains('b');  //返回值类型通过类型推论得到,可以省略不写
...
let htmlButtonElement = document.querySelector('#btn') as HTMLButtonElement;
...
function clickEvent(event: MouseEvent) {
    ...
    let target = event.target as HTMLButtonElement;  //通过类型断言获取到具体的DOM元素类型  其实this指向的就是该按钮,也就是DOM元素对象
